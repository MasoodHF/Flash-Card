...
char DOTS_CRLF_END_CHARACTERS[] = {'(', 'e', 'm', 'p', 't', 'y', ')','\n', '\0'}; 
	       /*----------------------------------------------------------*
...
char extra_main_buffer[MAIN_BUFFER_SIZE
...
int     get_line(char *output_buffer,
		 ...
		 const char *two_bytes_ending_format);
/******************************************************************************
 * - Opening files                                                            *
...
/******************************************************************************
 * - Editing a word, its definition & examples and saving it on the storage   *
 ...
 *                 : strlen(char *)                                           *
 *                 : printf(...)                                              *
 *                 : save(char *, unsigned long int, unsigned char *)         *
 ******************************************************************************//* ==>> PASSED */
void edit_word(void)
{
	size_t i;
...
void revise_words(void)
{
	...
	while (main_buffer_active_counter < strlen(main_buffer))
	{
		...
		{ /* found */
			...
			else if (short_buffer[0] == 'e' || short_buffer[0] == 'E')
			{
				goto exit;
			}
			else
...
void list_words(void) 
{
	...
	while (main_buffer_active_counter < length_of_main_buffer)
	{
		main_buffer_active_counter +=
			get_elements_of_record(read_record(main_buffer,
						main_buffer_active_counter, 6));
		printf("%4lu - %s", count + 1, word);
		count++;
	}
	printf("----------- End of %lu words list.\n", count);
}
/******************************************************************************
 * Global usage: buf, main_buffer_active_counter, str, unit,                  *
 ******************************************************************************//* ==>> PASSED */
...
/******************************************************************************
 *  Using globale variables and record buffer                                 *
 ******************************************************************************//* ==>> PASSED */
void set_record_ready_to_save(void)
{
	record_buffer[0] = '\0';
